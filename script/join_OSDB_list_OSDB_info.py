...
def join_OSDB_list_OSDB_info(df_OSDB_list, df_OSDB_infos, save_path, on_pair,
    ...
    df_OSDB_list_filtered.index.name = key_alias  # change index name from "Name" to "DBMS"
    df_OSDB_info_filtered.set_index(on_df_OSDB_info, inplace=True)
    df_OSDB_info_filtered.index.name = key_alias  # change index name from "Name" to "DBMS"
